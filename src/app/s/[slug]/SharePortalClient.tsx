...
async function buildPDF(plans: FloorPlan[], lang: Lang): Promise<jsPDF> {
  ...
  plans.forEach((plan, pi) => {
    ...
    doc.setFontSize(8);
    doc.setTextColor(156, 163, 175);
    doc.text("Generated by SplanAI", ML, PH - 5);
    doc.text(`© ${new Date().getFullYear()} SplanAI`, PW - ML, PH - 5, { align: "right" });
  });
...
export default function SharePortalClient({ slug, plans, clientName }: Props) {
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="mt-12 text-center">
          ...
          <p className="text-xs text-gray-300">
            {t.poweredBy} <span className="font-semibold text-gray-400">SplanAI</span>
          </p>
        </div>
      </div>
    </div>
