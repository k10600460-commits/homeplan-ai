...
async function fetchResearchAndAudit(
  ...
  try {
    // maxRetries:0 — web_search + Sonnet 5 research frequently exceeds 90s or hits
    // 529 overloaded. The SDK's DEFAULT 2 retries silently re-run the *billed*
    // web_search loop up to 3× per cron run, while trackedMessage only records the
    // final success → the retries are metered but invisible. This is the piece the
    // 2026-07-14 frequency right-sizing (54f2eae) missed: MTD console spend stayed
    // ~$64 despite the cuts because each failed research run still billed 2 retries
    // (only ~10 of 16 daily runs left a cost row). Fully guarded (returns
    // {result:null}); a single failed attempt just drops the day's research section.
...
良い記事が少ない日は無理に埋めず items を少なくしてよい。`;
    ...
    const msg = await trackedMessage("cron/daily-brief:research", anthropic, {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      tools: [
        { type: "web_search_20250305", name: "web_search", max_uses: 2 } as any, // 3→2: trim billed web_search input (cost fix 2026-07-16)
