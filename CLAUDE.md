...
## Session Hygiene（常時適用・全セッション共通）

目的: コンテキスト逼迫による品質低下を、起きる前にユーザーへ警告する。

1. 残量警告（こちらから発信する）
   - auto-compact警告の表示、/contextでの残量低下、大量ファイル読み込みの蓄積など
     コンテキスト逼迫の兆候を検知したら、作業を一旦止めて必ず告知する:
     「⚠️ コンテキスト残量が少なくなっています。区切りを推奨します」
   - 無断でauto-compactに突入したまま編集を続けない。

2. 区切る前のハンドオフ（必須）
   - /compact またはセッション終了を提案する前に、ハンドオフメモを更新する:
     obsidian-vault/SplanAI/50_Daily/session-handoff.md
     （完了したこと / 作業中のこと / 次の一手 / 未決事項 / 触れたファイルとブランチ）
   - その上で提案する: 同一タスク継続なら /compact、タスク完了なら新セッション。

3. 1セッション＝1タスク
   - 大きなタスクが完了したら、無関係な作業を続けず、セッションの終了を自分から提案する。
   - 大量読み込みを伴うタスクは開始前に「容量を食うため専用セッション推奨」と告知する。

## Copy Review
- 外向け英文（コールドメール/X/LP/LinkedIn）を生成・編集したら、提示前に /verify-copy 相当の自己検査を通す。
